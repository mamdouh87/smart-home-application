...
export class ProjectService {
  ...
  protected convertDateFromClient(project: IProject): IProject {
    return Object.assign({}, project, {
      enterDate: project.enterDate?.isValid() ? project.enterDate.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.enterDate = res.body.enterDate ? dayjs(res.body.enterDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((project: IProject) => {
        project.enterDate = project.enterDate ? dayjs(project.enterDate) : undefined;
      });
    }
    return res;
  }
